 ...
 * 
 * A PmuRegulatorConfig object is used to get/set modes for a regulator.
 * It holds the configuration that was read from / to be written to the PMU,
 * as well as a pointer to the regulator's PmuRegulatorCtrl object.
 * The PmuRegulatorCtrl object contains register addresses, bitfields and conversion functions
...
typedef struct PmuRegulatorCtrl {
...
typedef enum PmuOutputMode {
    PMU_OUTPUT_OFF,
    PMU_OUTPUT_ON
} PmuOutputMode;

typedef enum PmuDvmMode {
    PMU_DVM_OFF,
    PMU_DVM_ON
} PmuDvmMode;

typedef struct PmuRegulatorConfig {
    PmuOutputMode output;               // on (true), off (false)
    PmuDvmMode dvm;                     // on (true), off (false)
    uint16_t level_mv;                  // voltage level in mV
    PmuRegulatorCtrl *regulator;        // knows where to get/set
} PmuRegulatorConfig;


// DC/DC 3 is a buck converter (0.5-3.4V, max. 2A)
extern PmuRegulatorCtrl PMU_DC3;
...
extern PmuRegulatorCtrl PMU_BLDO1;
